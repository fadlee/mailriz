/**
 * mailvault-cli — interactive deployment wizard for MailVault.
 *
 * Commands:
...
const CONFIG_DIR = join(homedir(), '.mailvault');
const CONFIG_PATH = join(CONFIG_DIR, 'config.json');
const TMP_DIR = join(CONFIG_DIR, '.temp');
const RELEASE_URL = 'https://github.com/rizkirmdhnnn/mailvault/releases/latest/download/mailvault-worker.tar.gz';
...
async function fetchReleaseAsset(): Promise<{ dir: string; index: string; migrationsDir: string; assetsDir: string }> {
  ...
  const tarPath = join(TMP_DIR, 'mailvault-worker.tar.gz');
...
async function deployWithWrangler(opts: {
  ...
  const wranglerConfig = {
    ...
    d1_databases: [
      { binding: 'DB', database_name: 'mailvault', database_id: opts.d1Id, migrations_dir: 'migrations' },
    ],
    r2_buckets: [
...
async function cmdSetup() {
  ...
  let d1 = d1s.find((d) => d.name === 'mailvault');
  if (!d1) d1 = await createD1(token, accountId, 'mailvault');
  sp.stop(`✓ D1 ${d1.id}`);
  ...
  const r2Raw = r2s.find((b) => b.name === 'mailvault-raw') || await createR2Bucket(token, accountId, 'mailvault-raw');
  const r2Att = r2s.find((b) => b.name === 'mailvault-attachments') || await createR2Bucket(token, accountId, 'mailvault-attachments');
  const r2Html = r2s.find((b) => b.name === 'mailvault-html') || await createR2Bucket(token, accountId, 'mailvault-html');
  sp.stop('✓ R2 ready');

  // 8. Deploy worker (wrangler child process with generated wrangler.jsonc).
  sp.start('Deploying Worker…');
  const workerName = 'mailvault';
  try {
  ...
  try {
    ...
    sp.stop('✗');
    note(`Email Routing setup failed (${e.message}). Enable it manually in the dashboard: Email → Email Routing → Enable, then add a catch-all rule to Worker "mailvault".`, 'Manual step needed');
  }
  ...
  outro(
    ...
    pc.dim(`State saved to ~/.mailvault/config.json (chmod 600)`)
  );
}
...
async function cmdStatus() {
  intro(pc.bold('📊 MailVault status'));
  const cfg = await loadConfig();
  if (!cfg) fail('Not installed. Run `mailvault-cli setup` first.');
...
async function cmdUpdate() {
  intro(pc.bold('🔄 MailVault update'));
  const cfg = await loadConfig();
  if (!cfg) fail('Not installed. Run `mailvault-cli setup` first.');
...
async function cmdDestroy() {
  intro(pc.bold('💥 MailVault destroy'));
  const cfg = await loadConfig();
